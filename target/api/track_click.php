...
if (empty($igtrgt)) {
    ...
    echo json_encode(['success' => false, 'error' => 'igtrgt query parameter is required']);
    exit();
}
...
// Limit length and allow safe URL/Instagram slug characters
if (!preg_match('/^[a-z0-9._-]{1,50}$/', $igtrgt)) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Invalid igtrgt parameter']);
    exit();
}
